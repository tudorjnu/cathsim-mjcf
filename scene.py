...
class Scene(object):
    ...
    def __init__(self,
        ...
        option.viscosity = 3.5
        option.density = 997
        option.gravity = [0, 0, 0]
        option.flag.multiccd = 'enable'
        option.solver = 'Newton'            # PGS, CG, Newton
        option.integrator = 'Euler'      # Euler, RK4, implicit
        option.cone = 'pyramidal'           # pyramidal, elliptic
        option.jacobian = 'sparse'
        # option.noslip_iterations = 10
        # others
        self.model.size.njmax = 2000
        self.model.size.nconmax = 1000
        ...
        if xml_path is not None:
            ...
            model.default.geom.margin = 0.001
...
class Tip(object):
    ...
    def __init__(self,
        ...
        default.joint.stiffness = 4
        default.joint.armature = 0.05
        # actuator defaults
        default.velocity.ctrlrange = [-1, 1]
        default.velocity.forcerange = [-100, 100]
...
class Guidewire(object):
    ...
    def __init__(self,
                 ...
                 force: float = 100,  # force for translation and rotation
                 ):
        ...
        default.geom.type = 'capsule'
        default.geom.size = [sphere_radius, cylinder_height]
        default.geom.friction = [0.03, 0.03, 0.03]
        # joint defaults
        default.joint.type = 'hinge'
        default.joint.pos = [0, 0, -offset / 2]
        default.joint.ref = 0
        default.joint.damping = 0.005
        default.joint.stiffness = 0.4
        default.joint.armature = 0.05
        # actuator defaults
        default.velocity.ctrlrange = [-1, 1]
        default.velocity.forcerange = [-force, force]
        default.velocity.kv = 5
        # site
        default.site.type = 'sphere'
        default.site.size = [sphere_radius]

        worldbody = self.model.worldbody
        # set the first body
        parent = worldbody.add('body', name='guidewire_0',
                               euler=[-pi / 2, 0, pi], pos=[0, -self.length, 0],)
        parent.add('geom', type='capsule', name='capsule_0')
        ...
        gainprm = [kp, 0, 0]
        biasprm = [2]
        self.model.actuator.add('general', joint='rotator', name='rotator_actuator',
                                biastype=biastype, gaintype=gaintype, dyntype=dyntype,
                                dynprm=dynprm, gainprm=gainprm, biasprm=biasprm)

        # make the main body
        for i in range(n_bodies - 1):
            parent = self.add_body(parent, offset, ref=0)
    ...
    def add_body(self,
                 ...
                 damping: float = None,  # the damping of the joint
                 ):
        child_name = f'guidewire_{self.n}'
        child = parent.add('body', name=child_name, pos=[0, 0, offset])
        child.add('geom', name=f'capsule_{self.n}')
        j0 = child.add('joint', name=f'J0_{self.n}', axis=[1, 0, 0])
        j1 = child.add('joint', name=f'J1_{self.n}', axis=[0, 1, 0])
        if ref is not None:
            j1.ref = ref
...
catheter = Catheter()
scene.model.attach(catheter.mjcf_model)
